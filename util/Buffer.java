...
public class Buffer implements Serializable {
	...
	/**
	 * Ensures that there is enough length to store the specified number of bytes.
	 * If there isn't, enough extra room is allocated so we can.
	 *
	 * @param bytes
	...
	/**
	 * Shifts all data back by the requested number of bytes, and returns the bytes
	...
	public short removeWord() throws IndexOutOfBoundsException {
		int ret = ((removeByte() << 0) & 0x00FF) | ((removeByte() << 8) & 0xFF00);
	...
	public long removeLong() throws IndexOutOfBoundsException {
		return (((long) removeByte() << 0L) & 0x00000000000000FFL) | (((long) removeByte() << 8L) & 0x000000000000FF00L)
	...
	/**
	 * Adds another buffer to the end of the buffer. All it actually does is add the
	 * bytes of the source to the current buffer.
	 * 
	 * @param b
	 *            The buffer to add.
	 */
	...
	public String toString() {
		...
		for (i = 0; i < currentLength; i++) {
			...
			returnString.append(
					Integer.toString((buffer[i] & 0xF0) >> 4, 16) + Integer.toString((buffer[i] & 0x0F) >> 0, 16));
			returnString.append(' ');
		}
