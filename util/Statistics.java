...
public class Statistics {
	...
	public void connect() {
		...
		db = new Database();
		if (db.connect("jdbc:mysql://" + Constants.StatsServer + "/" + Constants.StatsDatabase, Constants.StatsUsername,
				Constants.StatsPassword)) {
			Out.info("Stats", "Connected to SQL server");
		} else {
			Out.info("Stats", "Failed to connect to Database " + Constants.StatsDatabase + " on "
					+ Constants.StatsServer + " as " + Constants.StatsUsername);
		}
		checkStruct();
	}

	private void checkStruct() {
		if (!Constants.StatsCheckSchema)
			return;
		if (db == null)
			return;
		try {
			...
			ResultSet r = smnt
					.executeQuery("SELECT `TABLE_NAME` FROM INFORMATION_SCHEMA.TABLES WHERE `TABLE_SCHEMA` = '"
							+ Constants.StatsDatabase + "'");
			while (r.next()) {
				String name = r.getString("TABLE_NAME");
				if (name.equals("checkrevisions"))
					crev = true;
				else if (name.equals("botlogins"))
					logins = true;
			...
			if (!crev) {
				...
				smnt.executeUpdate(
						"CREATE TABLE `checkrevisions` (`game` INT NOT NULL, `version` VARCHAR( 255 ) NOT NULL, `time` TIMESTAMP NOT NULL, `ip` VARCHAR( 40 ) NOT NULL, `botID` VARCHAR( 255 ) NOT NULL) ENGINE = MYISAM;");
			}
			if (!logins) {
				Out.info("Stats", "Login table does not exist, Creating...");
				smnt.executeUpdate(
						"CREATE TABLE `botlogins` (`time` TIMESTAMP NOT NULL, `ip` VARCHAR( 40 ) NOT NULL, `botID` VARCHAR( 255 ) NOT NULL) ENGINE = MYISAM;");
			}
			if (!connections) {
				Out.info("Stats", "Connection table does not exist, Creating...");
				smnt.executeUpdate(
						"CREATE TABLE `connections` (`time` TIMESTAMP NOT NULL, `ip` VARCHAR( 40 ) NOT NULL) ENGINE = MYISAM;");
			}
			smnt.close();
			r = null;
			smnt = null;
