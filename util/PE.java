...
public class PE {
	...
	/**
	 * This is the entry point to the whole file version function set. It takes the
	 * filename as a parameter and returns the version. If the version can't be
	 * returned for whatever reason, an IOException is thrown.
	 *
	 * @param filename
	...
	public static int getVersion(String filename, boolean byteorder) throws IOException {
		/*
		 * psStart is the pointer to the first byte in the PE data. The byte is pointed
		 * to at 0x3c
		 */
		int peStart;
		...
		/*
		 * A pointer to the "optional" header (which will always be present in .exe
		 * files
		 */
		int ptrOptionalHeader;

		/* The file we're reading from. */
		MappedByteBuffer file = new FileInputStream(filename).getChannel().map(FileChannel.MapMode.READ_ONLY, 0,
	 ...
	 */
	private static int processOptionalHeader(MappedByteBuffer file, int ptrOptionalHeader, int numberOfSections,
			boolean byteorder) throws IOException {
		/*
		 * Set to true if this is a PE+ file. Some addresses are slightly different.
		 * Untested!
		 */
		boolean plus;
		/*
		 * The number of RVA entries. We don't care what the entries are, so we just
		 * skip right over them.
		 */
		int numberOfRvaAndSizes;
		/*
		 * A pointer to the table of sections. This, along with the number of sections,
		 * is passed to the next function.
		 */
		int ptrSectionTable;
	 ...
	 */
	private static int processSections(MappedByteBuffer file, int sectionsBase, int numberOfSections, boolean byteorder)
			throws IOException {
		...
		for (int i = 0; i < numberOfSections; i++) {
			...
			/*
			 * Find the difference between the actual location of rsrc and the virtual
			 * location of it
			 */
			...
			if (file.getLong(sectionsBase + (i * 40)) == rsrc)
				return processResourceRecord(new LinkedList<Integer>(), file, 0, file.getInt(sectionBase + 20),
						rsrcVirtualToRaw, byteorder);
		}
	 ...
	 * processEntry which, in turn, calls it. The function looks specifically for
	 * the version section. As soon as it finds a leaf node for a version section,
	 * it returns the data all the way back up the resursive stack. The recursion
	 ...
	 * @param recordOffset
	 *            The offset of the record that we're going to process. It's added
	 *            to the rsrcStart value to get a pointer.
	 * @param rsrcStart
	 *            The very beginning of the rsrc section. Used as a base value.
	 * @param rsrcVirtualToRaw
	 *            The value that has to be added to the virtual section to get the
	 *            raw section.
	 * @return The version, or 0 if it couldn't be found.
	 * @throws IOException
	 *             If there's an error finding the version.
	 */
	private static int processResourceRecord(LinkedList<Integer> tree, MappedByteBuffer file, int recordOffset,
			int rsrcStart, int rsrcVirtualToRaw, boolean byteorder) throws IOException {
		int i;
		...
		for (i = 0; i < numberIDEntries; i++) {
			...
			/*
			 * Process the entry. processEntry() will call processResourceRecord() again for
			 * branches.
			 */
	...
	/**
	 * Process an entry recursively. If a leaf node is found, and it's the version
	 * node, return it.
	 *
	 * @param tree
	...
	private static int processEntry(LinkedList<Integer> tree, MappedByteBuffer file, int entry, int rsrcStart,
		...
		/*
		 * The address of the next node, or the address of the data. The left-most bit
		 * tells us which address this actually is.
		 */
		...
		/*
		 * Check if it's a branch by checking the left-most bit. If it's set, it's a
		 * branch.
		 */
		if ((nextAddress & 0x80000000) != 0) {
			...
			if (tree.get(0) == RT_VERSION) {
				...
				if (byteorder)
					...
					return buffer[0x3A] << 24 | buffer[0x38] << 16 | buffer[0x3E] << 8 | buffer[0x3C] << 0;
			}
		}
	...
	public static void main(String[] args) throws IOException {
		/*
		 * System.out.println(String.format("%08x", getVersion("./STAR/starcraft.exe",
		 * false))); System.out.println(String.format("%08x",
		 * getVersion("./W2BN/Warcraft II BNE.exe", true)));
		 * System.out.println(String.format("%08x", getVersion("./WAR3/war3.exe",
		 * true))); System.out.println(String.format("%08x",
		 * getVersion("./D2DV/Game.exe", false)));
		 * System.out.println(String.format("%08x", getVersion("./DRTL/Diablo.exe",
		 * false))); System.out.println(String.format("%08x",
		 * getVersion("./DSHR/Diablo_s.exe", false)));
		 * System.out.println(String.format("%08x", getVersion("./JSTR/StarCraftJ.exe",
		 * false))); System.out.println(String.format("%08x",
		 * getVersion("./SSHR/StarCraft_s.exe", false)));
		 * System.out.println(String.format("%08x", getVersion("./JSTR/StarCraftJ.exe",
		 * false))); int tmpWAR3 = getVersion("./war3/war3.exe", true);
		 * 
		 * int WAR3 = (tmpWAR3 & 0xFF000000) >>> 24 | (tmpWAR3 & 0x00FF0000) >> 8 |
		 * (tmpWAR3 & 0x0000FF00) << 8 | (tmpWAR3 & 0x000000FF) << 24;
		 ...
		 * System.out.println(Constants.versionHashD2DV == getVersion("./D2DV/Game.exe",
		 * false)); System.out.println(Constants.versionHashD2XP ==
