		...
		try {
			...
			while ((var = inputStream.readLine()) != null) {
				if (var.toLowerCase().equals("[" + Header.toLowerCase() + "]"))
					...
					break;
				else if (var.length() >= Setting.length() + 1 && fndHdr == true
						&& var.toLowerCase().substring(0, Setting.length()).equals(Setting.toLowerCase()))
			...
			while ((var = inputStream.readLine()) != null) {
				if (fndHdr == false)
					StringB.append(var + "\n");
						...
						&& (var.toLowerCase().substring(0, Setting.length() + 1).equals(Setting.toLowerCase() + "=")))
					fndHdr = false;
				else
					StringB.append(var + "\n");
				if (var.toLowerCase().equals("[" + Header.toLowerCase() + "]")) {
