...
public class PEFile {
	...
	private void fillIAT(IMAGE_DATA_DIRECTORY dir) {
		...
		while (iid.AddressOfName != 0) {
			...
			while (getIntSection(iid.AddressOfImportLookupTable + (4 * y)) != 0) {
				...
				// System.out.println(Integer.toHexString(address) + ": " + function + ": " +
	...
	private void loadSection(IMAGE_SECTION_HEADER section, char[] buffer) {
		char[] data = getData(section.PointerToRawData,
				(section.SizeOfRawData > section.VirtualSize ? section.VirtualSize : section.SizeOfRawData));
	...
	public PEFile(String fn) {
		...
		int sectionStart = ntheader.FileHeader.SizeOfOptionalHeader + dosheader.e_lfanew + SIZE_OF_NT_SIGNATURE
	...
	public void putSection(int data, int offset) {
		...
		// System.out.println("Changing: 0x" + Integer.toHexString(offset) + " to 0x" +
