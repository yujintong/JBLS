...
public class cSettings {
	...
	public static void LoadSettings() {
		...
		Constants.BNLSPort = Integer
				.parseInt(Ini.ReadIni(file, "Main", "BNLSPort", Integer.toString(Constants.BNLSPort)), 10);
		Constants.maxThreads = Integer
				.parseInt(Ini.ReadIni(file, "Main", "MaxThreads", Integer.toString(Constants.maxThreads)), 10);

		Constants.RunHTTP = Boolean
				.valueOf(Ini.ReadIni(file, "Main", "EnableHTTP", Boolean.toString(Constants.RunHTTP)));
		Constants.HTTPPort = Integer
				.parseInt(Ini.ReadIni(file, "Main", "HTTPPort", Integer.toString(Constants.HTTPPort)), 10);

		Constants.ipAuthStatus = Integer
				.parseInt(Ini.ReadIni(file, "Main", "IPAuth", Integer.toString(Constants.ipAuthStatus)), 10);
		Constants.requireAuthorization = Boolean
				.valueOf(Ini.ReadIni(file, "Main", "RequireAuth", Boolean.toString(Constants.requireAuthorization)));

		Constants.displayPacketInfo = Boolean
				.valueOf(Ini.ReadIni(file, "Main", "DisplayPacketInfo", Boolean.toString(Constants.displayPacketInfo)));
		Constants.displayParseInfo = Boolean
				.valueOf(Ini.ReadIni(file, "Main", "DisplayParseInfo", Boolean.toString(Constants.displayParseInfo)));
		Constants.debugInfo = Boolean
				.valueOf(Ini.ReadIni(file, "Main", "DisplayDebugInfo", Boolean.toString(Constants.debugInfo)));
		Constants.enableLogging = Boolean
				.valueOf(Ini.ReadIni(file, "Main", "EnableLogging", Boolean.toString(Constants.enableLogging)));
		Constants.logKeepDuration = Integer
				.parseInt(Ini.ReadIni(file, "Main", "LogFileKeepDays", Integer.toString(Constants.logKeepDuration)));

		Constants.RunAdmin = Boolean
				.valueOf(Ini.ReadIni(file, "Admin", "EnableAdmin", Boolean.toString(Constants.RunAdmin)));
		...
		Constants.LogStats = Boolean
				.valueOf(Ini.ReadIni(file, "Statistics", "Enable", Boolean.toString(Constants.LogStats)));
		Constants.StatsLogIps = Boolean
				.valueOf(Ini.ReadIni(file, "Statistics", "Include IPs", Boolean.toString(Constants.StatsLogIps)));
		Constants.StatsLogConns = Boolean
				.valueOf(Ini.ReadIni(file, "Statistics", "Connections", Boolean.toString(Constants.StatsLogConns)));
		Constants.StatsLogCRevs = Boolean
				.valueOf(Ini.ReadIni(file, "Statistics", "CheckRevisions", Boolean.toString(Constants.StatsLogCRevs)));
		Constants.StatsLogBotIDs = Boolean
				.valueOf(Ini.ReadIni(file, "Statistics", "BotIDs", Boolean.toString(Constants.StatsLogBotIDs)));
		Constants.StatsCheckSchema = Boolean
				.valueOf(Ini.ReadIni(file, "Statistics", "Check Schema", Boolean.toString(Constants.StatsCheckSchema)));
		Constants.StatsQueue = Integer
				.parseInt(Ini.ReadIni(file, "Statistics", "Queue", Integer.toString(Constants.StatsQueue)), 10);
		...
		for (int x = 0; x < Constants.prods.length; x++) {
			Constants.IX86files[x][0] = Ini.ReadIni(file, Constants.prods[x] + "-IX86", "HashPath",
					Constants.IX86files[x][0]);
			Constants.IX86files[x][1] = Ini.ReadIni(file, Constants.prods[x] + "-IX86", "Exe",
					Constants.IX86files[x][1]);
			Constants.IX86files[x][2] = Ini.ReadIni(file, Constants.prods[x] + "-IX86", "Storm",
					Constants.IX86files[x][2]);
			Constants.IX86files[x][3] = Ini.ReadIni(file, Constants.prods[x] + "-IX86", "Network",
					Constants.IX86files[x][3]);
			Constants.IX86files[x][4] = Ini.ReadIni(file, Constants.prods[x] + "-IX86", "Screen",
					Constants.IX86files[x][4]);
			Constants.IX86verbytes[x] = Integer.parseInt(Ini.ReadIni(file, Constants.prods[x] + "-IX86", "VerByte",
					Integer.toHexString(Constants.IX86verbytes[x])), 16);
		}
	}
