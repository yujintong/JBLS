...
public class IntFromByteArray {
	...
	/*
	 * public static void main(String args[]) { byte[] test = { 1, 2, 3, 4, 5, 6, 7,
	 * 8, 9 };
	 * 
	...
	public int getInteger(byte[] array, int location) {
		if ((location + 3) >= array.length)
			throw new ArrayIndexOutOfBoundsException("location = " + location + ", number of bytes = " + array.length
	 ...
	 * array. This is used to simulate pointers used in C++. Note that this works in
	 * little endian only.
	 * 
	 * @param intBuffer
	...
	public byte[] insertInteger(byte[] array, int location, int b) {
		if (location + 3 >= array.length)
			throw new ArrayIndexOutOfBoundsException("location = " + location + ", length = " + array.length
