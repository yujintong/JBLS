...
public class BNFTP extends Thread {
	...
	public void run() {
		download(server, port, file, bn, id);
	}
	...
	public String download(String bnServer, int bnPort, String fileName, BotNet callback, int userid) {
		try {
			...
			if (callback != null)
				callback.whisper(userid, "BNFTP: Sending request for " + fileName);
			...
			try {
				...
				if (callback != null)
					callback.whisper(userid, "BNFTP: " + fileName + " Recived File Size " + fileSize);
				...
				if (callback != null)
					callback.whisper(userid, "BNFTP: Recived file: " + fileName + " Size: " + fileSize);
				return "Recived file: " + fileName + " Size: " + fileSize;
			} catch (IOException e) {
				Out.println("BNFTP", "IOError: " + e.toString());
				if (callback != null)
					callback.whisper(userid, "BNFTP: " + fileName + " IOError: " + e.toString());
			...
			if (callback != null)
				callback.whisper(userid, "BNFTP: Could not find host: " + bnServer + ":" + bnPort);
