...
public class HTTPParse extends Thread {
	...
	private String defaultCSS = "body{" + CRLF + "  background-color: #000000;" + CRLF + "  background-repeat: repeat;"
			+ CRLF + "  text-align: center;" + CRLF + "  font-family: \"Arial\";" + CRLF + "  font-size: 12px;" + CRLF
			+ "  color: #ffffff;" + CRLF + "}" + CRLF + "span.mainheader{font-size: 20px;}" + CRLF
			+ ".header {font-weight: bold;}" + CRLF + "span.mainsub {font-style: italic;}" + CRLF
			+ "span.subheader {font-weight: bold;}" + CRLF + ".indent {padding-left: 50pt;}" + CRLF + ".line {" + CRLF
			+ "  font-size: 12px;" + CRLF + "  text-align: center;" + CRLF + "  padding-top: 5px;" + CRLF
			+ "  padding-bottom: 5px;" + CRLF + "  vertical-align: top;" + CRLF + "  border-bottom: 1px solid #ffffff;"
			+ CRLF + "}" + CRLF + ".li {" + CRLF + "  font-size: 12px;" + CRLF + "  text-align: center;" + CRLF
			+ "  padding-top: 3px;" + CRLF + "  padding-bottom: 3px;" + CRLF + "  border-bottom: 1px dotted #ffffff;"
			+ CRLF + "}";

	private String defaultPage = "<html>" + CRLF + "  <head>" + CRLF + "    <title>JBLS HTTP SERVER INFO</title>" + CRLF
			+ "    <link href='style.css' rel='stylesheet' type='text/css' />" + CRLF + "  </head>" + CRLF + "  <body>"
			+ CRLF + "    <span class=mainheader>JBLS HTTP SERVER INFORMATION</span><br />" + CRLF
			+ "    <span class=mainsub>by HdX</span>" + CRLF + "    <hr width=325 color=#ffffff>" + CRLF
			...
			+ "    </table>" + CRLF + "    <table width=250 align=center cellpadding=3 cellspacing=0 border=0>" + CRLF
			+ "      <tr>" + CRLF
			...
			+ "        <td align=left class=line width=100><strong>Version Byte</strong></td>" + CRLF + "      </tr>"
			+ CRLF + "      <tr><td class=li>STAR Keys</td><td class=li><sck></td><td class=li>-</td></tr>" + CRLF
			...
			+ "    </table>" + CRLF + "    <table width=550 align=center cellpadding=3 cellspacing=0 border=0>" + CRLF
			...
			+ "      <tr><td><build></td></tr>" + CRLF + "    </table>" + CRLF + "  </body>" + CRLF + "</html>";
	...
	public void run() {
		...
		try {
			...
			if (reqType.equalsIgnoreCase("GET")) {
				...
				if (file.equalsIgnoreCase("/") || file.equalsIgnoreCase("/index.htm")
					...
					page = page.replaceAll("<d2xpvb>",
							hex(Constants.IX86verbytes[Constants.PRODUCT_LORDOFDESTRUCTION - 1]));
					page = page.replaceAll("<jstrvb>",
							hex(Constants.IX86verbytes[Constants.PRODUCT_JAPANSTARCRAFT - 1]));
					...
					page = page.replaceAll("<dshrvb>",
							hex(Constants.IX86verbytes[Constants.PRODUCT_DIABLOSHAREWARE - 1]));
					page = page.replaceAll("<sshrvb>",
							hex(Constants.IX86verbytes[Constants.PRODUCT_STARCRAFTSHAREWARE - 1]));
					page = page.replaceAll("<auth>", String.valueOf(Constants.requireAuthorization));
					page = page.replaceAll("<build>", Constants.build);
					page = page.replaceAll("<news>",
							Constants.strNews[0] + "<br \\>" + Constants.strNews[1] + "<br \\>" + Constants.strNews[2]
									+ "<br \\>" + Constants.strNews[3] + "<br \\>" + Constants.strNews[4]);
					writeBytes(
							HTTPServer.buildHeader(200, "text/html; charset=iso-8859-1", page.length()) + CRLF + page);
					...
					writeBytes(HTTPServer.buildHeader(200, "text/plain", 40) + CRLF + util.PadString.padHex(hash[0], 8)
							+ util.PadString.padHex(hash[1], 8) + util.PadString.padHex(hash[2], 8)
							+ util.PadString.padHex(hash[3], 8) + util.PadString.padHex(hash[4], 8));

				} /*
					 * else if(file.equalsIgnoreCase("/WAR3.zip")) sendHashFile("WAR3",
					 * Constants.WAR3files); else if(file.equalsIgnoreCase("/W2BN.zip"))
					 * sendHashFile("W2BN", Constants.W2BNfiles); else
					 * if(file.equalsIgnoreCase("/STAR.zip")) sendHashFile("STAR",
					 * Constants.STARfiles); else if(file.equalsIgnoreCase("/D2DV.zip"))
					 * sendHashFile("D2DV", Constants.D2DVfiles); else
					 * if(file.equalsIgnoreCase("/D2XP.zip")) sendHashFile("D2XP",
					 * Constants.D2XPfiles); else if(file.equalsIgnoreCase("/JSTR.zip"))
					 * sendHashFile("JSTR", Constants.JSTRfiles); else
					 * if(file.equalsIgnoreCase("/DRTL.zip")) sendHashFile("DRTL",
					 * Constants.DRTLfiles); else if(file.equalsIgnoreCase("/DSHR.zip"))
					 * sendHashFile("DSHR", Constants.DSHRfiles); else
					 * if(file.equalsIgnoreCase("/SSHR.zip")) sendHashFile("SSHR",
					 * Constants.SSHRfiles);
					 */
				else {
					send404(file);
				...
				String page = "<http>" + CRLF + "  <head>" + CRLF + "    <title>JBLS HTTP Server Error!</title>" + CRLF
						+ "    <link href=\"style.css\" type=\"text/css\" rel=\"stylesheet\">" + CRLF + "  </head>"
						+ CRLF + "  <body>" + CRLF + "    <h1>JBLS's HTTP server does not support this function.</h1>"
						+ CRLF + "    <h6>Server By Hdx.</h6>" + CRLF + "  </body>" + CRLF + "</http>";
	...
	private void send404(String file) {
		String page = "<html>" + CRLF + "  <head>" + CRLF + "    <title>File Not Found</title>" + CRLF
				+ "    <link href=\"style.css\" type=\"text/css\" rel=\"stylesheet\">" + CRLF + "  </head>" + CRLF
				+ "  <body>" + CRLF + "    <h1>Sorry but the file <i>" + file
				+ "</i> could not be found on this server.</h1>" + CRLF + "  </body>" + CRLF + "</html>";
