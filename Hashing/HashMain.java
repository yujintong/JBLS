...
public class HashMain {
	...
	public static CheckrevisionResults getRevision(int prod, String formula, String dll, byte platform, int ver) {
		...
		try {
			CRevChecks[prod - 1]++;
			if (Constants.displayParseInfo)
				Out.info("HashMain", ">>> [" + Constants.prods[prod - 1] + "] Version Check V" + ver);
			switch (ver) {
			case 1:
				files = CheckRevisionV1.getFiles(prod, platform);
				return CheckRevisionV1.checkRevision(formula, prod, platform, dll);
			case 2:
				files = CheckRevisionV1.getFiles(prod, platform);
				return CheckRevisionV2.checkRevision(formula, prod, platform, dll);
			case 3:
				files = CheckRevisionV3.getFiles(prod, platform);
				return CheckRevisionV3.checkRevision(formula, prod, platform, dll);
			default:
				files = CheckRevisionV1.getFiles(prod, platform);
				return CheckRevisionV2.checkRevision(formula, prod, platform, dll);
			}
			...
			if (ver == 3)
				Out.error("HashMain",
						"Hash Exception(version check): \n\r" + "[CheckRevision] Files Not Found/Accessible ("
								+ Constants.prods[prod - 1] + ") (" + fileList + ", " + Constants.ArchivePath
								+ dll.replaceAll("mpq", "dll") + ")");
			else
				Out.error("HashMain",
						"Hash Exception(version check): \n\r" + "[CheckRevision] Files Not Found/Accessible ("
								+ Constants.prods[prod - 1] + ") (" + fileList + ")");
		} catch (IOException e) {
			Out.error("HashMain",
					"Hash Exception(version check): [CheckRevision] IOException (" + Constants.prods[prod - 1] + ")");
		}
		return null;
	}
	...
	public static CheckrevisionResults getRevision(int prod, String formula, String dll, long filetime) {
		...
		if (dll.matches("ver-PMAC-[0-7].mpq") == true)
			return getRevision(prod, formula, dll, Constants.PLATFORM_POWERPC, 2);
		...
		if (dll.matches("PMACver[0-7].mpq") == true)
			return getRevision(prod, formula, dll, Constants.PLATFORM_POWERPC, 1);
