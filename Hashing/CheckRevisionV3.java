...
public class CheckRevisionV3 extends CheckRevisionV1 {
	...
	public static CheckrevisionResults checkRevision(String valuestring, int prod, byte plat, String mpq)
		...
		for (int x = 0; x < temp_memory.length; x++)
			returnbuff.add((byte) temp_memory[x]);
		returnbuff.addByte((byte) 0);
		if (Version[plat][prod] == 0)
			Version[plat][prod] = getVersion(files, prod);

		CheckrevisionResults result = new CheckrevisionResults(Version[plat][prod], ld_sha1_out_buffer_2[0],
				returnbuff);
		crCache.put(valuestring + mpq.replaceAll("dll", "mpq") + prod + plat, result);
		return result;
	}

	public static void hash_file(lockdown_SHA1 ctx, String filename, String lockdown)
			throws FileNotFoundException, IOException {
		...
		if (pe.ntheader.Signature != PEFile.IMAGE_NT_SIGNATURE)
	...
	public static void hash1(lockdown_SHA1 ctx, lockdown_heap ld_heap, PEFile pe, int preferred_baseaddr,
			int preferred_baseaddr_upper, IMAGE_SECTION_HEADER section, int section_alignment, String lockdown) {
		...
		if (section.Characteristics < 0) {
			...
			if (section.VirtualSize > 0) {
				...
				while ((ptr_memory - starting_memory) < section.VirtualSize) {
					...
					if (section_length > 0) {
						// System.out.println("Section_length_update: " + PadString.padHex(ptr_memory,
						// 8) + " " + PadString.padHex(section_length, 8));
			...
			if (dwBytes > 0) {
				...
				while (i < dwBytes) {
					...
					if (index < ld_heap.length()) {
						...
						eax = (int) (var_40[0] - section.VirtualSize - section.VirtualAddress
								+ allocated_memory_base[0]);
					}
					dwBytes += i;
					if (eax > 0) {
					...
					if (dwBytes > 0) {
						...
						hash2(pe, preferred_baseaddr, preferred_baseaddr_upper, ctx, var_30, allocated_memory_base[i],
								lockdown);
						index++;
						i += var_30[4];
					}
	...
	private static void hash2(PEFile pe, int preferred_baseaddr, int preferred_baseaddr_upper, lockdown_SHA1 ctx,
		...
		int seedIndex = Integer
				.parseInt(lockdown.substring(lockdown.toLowerCase().indexOf("lockdown-ix86-")).substring(14, 16));

		if (seeds[seedIndex][0] == 0 || seeds[seedIndex][1] == 0)
			seeds[seedIndex] = seed_finder.find_seeds(lockdown, seed_finder.lockdown_values,
					seed_finder.lockdown_offsets);
		...
		if (offset_memory[2] == 0) {
			if (offset_memory[3] == 0) {
				...
				// System.out.print("0: ");utile.print_hash(pe.getDataSection(offset_memory[3],
				// offset_memory[1]));
